...
| `isSignedIn` | <code>undefined \| boolean</code>              | A boolean that returns `true` if the user is signed in.                                                                         |
| `user`       | <code>undefined \| null \| UserResource</code> | The [`User`](/docs/references/javascript/user) object for the current user. If the user isn't signed in, `user` will be `null`. |
